...
//! run within the 100 Hz control budget (`config::CONTROL_PERIOD_MS`).
//!
//! Uses synthetic (pseudo-random, deterministic) observations instead of
//! real sensors -- this only measures NN inference latency, not sysfs
//! sensor I/O.
//!
...
use ev3_balance_bot_firmware::config::{CONTROL_PERIOD_MS, INPUT_DIM};
...
fn main() {
    ...
    for _ in 0..BENCH_ITERS {
        ...
        let _action = std::hint::black_box(policy::infer(&obs));
    ...
    println!("  control budget (100 Hz): {budget:?}");
    println!("  mean:  {mean:?}");
    println!("  p50:   {p50:?}");
    println!("  p99:   {p99:?}");
    println!("  worst: {worst:?}");
