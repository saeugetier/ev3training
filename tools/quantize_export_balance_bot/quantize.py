...
def quantize_bias_s64(bias: np.ndarray, input_scale: float, weight_scale: float) -> np.ndarray:
  """Biases are quantized to int64 at `input_scale * weight_scale`, matching
  `fully_connected_s16`'s `bias: Option<&[i64]>` parameter.

  Verified against the installed crate (`fully_connected.rs`): bias is added
  to the accumulator in the *same* raw dot-product units *before* the
  kernel's internal `acc >> 15` step, and the combined `(dot + bias) >> 15`
  is requantized with the *same* multiplier used for the dot product (whose
  `*32768` factor already compensates for that shared shift -- see
  `fc_s16_requant_params`). Giving bias its own extra `/32768` factor here
  double-compensates the shift and makes the bias term ~32768x too large
  (verified empirically: it saturates every activation to +-32767).

  Note this does mean a bias is only resolved once `bias / (input_scale *
  weight_scale)` exceeds roughly 32768 in magnitude; for near-zero dot
  products (e.g. an all-zero probe input) a small bias can be floored to 0
  by the shared `>> 15` before it reaches the multiplier -- an inherent
  precision floor of this kernel for that edge case, not a scale bug.
  """
