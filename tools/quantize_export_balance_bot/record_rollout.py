...
checkpoint from a wandb run path, extracting the actor module) is not yet
verified against an installed mjlab/rsl_rl version. Adjust the
...
def load_policy(wandb_run_path: str):
  """Load a trained actor from an rsl_rl checkpoint fetched via its
  mjlab/wandb run path.
